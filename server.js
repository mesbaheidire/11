...
قواعد صارمة:
1. استخرج اسم الهاتف بالضبط كما هو معروف تجارياً (مثال: Samsung Galaxy S24 Ultra, iPhone 16 Pro Max, Xiaomi 14 Ultra, Redmi Note 13 Pro+ 5G, POCO X8 PRO 5G).
2. يجب أن يتضمن: الشركة المصنعة + الموديل + الرقم + اللاحقة (Pro, Ultra, Plus, Max, Lite, 5G) إن وجدت.
3. أضف مواصفات RAM/ROM إذا ذُكرت في النص بشكل واضح (مثال: 8GB/256GB).
4. لا تترجم الاسم — اكتبه بالإنجليزية دائماً.
5. ابحث عن كل الكلمات المهمة: POCO, Xiaomi, Samsung, iPhone, أوبو، إتش تي سي، إلخ
6. تأكد من شمول اللاحقات مثل: 5G, Pro, Max, Plus, Ultra, Pro Max
7. أعد فقط JSON: {"phoneName":"اسم الهاتف الكامل"} أو {"phoneName":null} إذا لم تجد هاتفاً.
8. لا تضف أي شرح — فقط JSON.
...
app.post('/api/ai-extract-seller-coupon', async (req, res) => {
  try {
    ...
    const prompt = `أنت خبير في استخراج قسائم البائع من نصوص منشورات المتاجر بالعربية والإنجليزية.
ركز على استخراج قسيمة أو كود متعلق بـ "البائع" أو "المتجر" تحديداً، مثل:
- "قسيمة البائع: SUV5QSCYTUHK"
- "قسيمة البائع : 100X8PPC"
- "100X8PPC $100/400"
- "عرض متجر البائع: ABC123"
- "خصم من متجر البائع: XYZ789"
- "قسيمة من صفحة المنتج: CODE"
- "قسيمة البائع" أو "Seller Coupon"

قواعد البحث:
1. ابحث عن كلمات مثل: "قسيمة البائع", "قسيمة متجر", "عرض متجر", "خصم البائع"
2. الكود عادة يكون مزيج من أحرف وأرقام (مثل: 100X8PPC, SUV5QSCYTUHK, ABC123, XYZ789)
3. قد يكون متبوع بـ مبلغ (مثل $100/400 أو $80/400)
4. تجاهل الأكواد العامة "كوبون" مثل ZNQ007, AFAS7 إلا إذا كانت مصرح بأنها "قسيمة البائع"

النص:
${text}

استخرج فقط كود قسيمة البائع الواضح (إن وجد).

رد بـ JSON فقط (بدون markdown):
{"sellerCoupon": "كود قسيمة البائع أو null"}`;
