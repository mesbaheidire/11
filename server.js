...
const storeSectionQueries = {
  hot: [
    { keywords: 'wireless earbuds bluetooth', category: '44' },
    { keywords: 'smart watch fitness tracker', category: '44' },
    { keywords: 'phone case cover', category: '509' },
    { keywords: 'LED light strip RGB', category: '39' },
    { keywords: 'portable charger power bank', category: '44' }
  ],
  deals: [
    { keywords: 'USB cable charger fast charging', maxPrice: '5' },
    { keywords: 'phone holder stand mount', maxPrice: '5' },
    { keywords: 'earphone wired headset', maxPrice: '5' },
    { keywords: 'screen protector tempered glass', maxPrice: '5' },
    { keywords: 'LED flashlight mini torch', maxPrice: '5' }
  ],
  phones: [
    { keywords: 'smartphone android mobile 5G', category: '509' },
    { keywords: 'POCO Xiaomi Redmi phone', category: '509' },
    { keywords: 'Samsung Galaxy smartphone', category: '509' },
    { keywords: 'Realme smartphone', category: '509' }
  ]
};

async function fetchMixedProducts(queryList, limitPerQuery) {
  const allProducts = [];
  const seenIds = new Set();
  for (const q of queryList) {
    try {
      const opts = { keywords: q.keywords, limit: String(limitPerQuery || 4), sort: 'LAST_VOLUME_DESC' };
      if (q.category) opts.category = q.category;
      if (q.maxPrice) opts.maxPrice = q.maxPrice;
...
app.get('/api/store/hot', async (req, res) => {
  try {
    const queries = storeSectionQueries.hot;
    const picked = queries.sort(() => Math.random() - 0.5).slice(0, 3);
    const products = await fetchMixedProducts(picked, 4);
    if (!products.length) {
      const fallback = await searchHotProducts({ limit: '10' });
      return res.json({ success: true, products: fallback.success ? fallback.products : [] });
    }
...
app.get('/api/store/deals', async (req, res) => {
  try {
    const queries = storeSectionQueries.deals;
    const picked = queries.sort(() => Math.random() - 0.5).slice(0, 3);
    const products = await fetchMixedProducts(picked, 4);
    if (!products.length) {
      const fallback = await searchProducts({ limit: '10', maxPrice: '5', sort: 'LAST_VOLUME_DESC' });
      return res.json({ success: true, products: fallback.success ? fallback.products : [] });
    }
    res.json({ success: true, products });
  } catch (e) {
...
app.get('/api/store/top-discount', async (req, res) => {
  try {
    const discountQueries = [
      { keywords: 'sale clearance discount electronics' },
      { keywords: 'flash deals smart watch earbuds' },
      { keywords: 'promotion offer gadgets accessories' }
    ];
    const products = await fetchMixedProducts(discountQueries, 8);
    const withDiscount = products.filter(p => {
      if (p.discount) return true;
      if (p.original_price && p.sale_price) {
        ...
        if (orig > 0 && sale > 0 && sale < orig) {
          p.discount = Math.round((1 - sale / orig) * 100) + '%';
          return true;
        }
      }
      return false;
    ...
    res.json({ success: true, products: withDiscount.slice(0, 10) });
  } catch (e) {
    res.json({ success: false, products: [] });
  }
});
...
app.get('/api/store/phones', async (req, res) => {
  try {
    const queries = storeSectionQueries.phones;
    const picked = queries.sort(() => Math.random() - 0.5).slice(0, 2);
    const products = await fetchMixedProducts(picked, 6);
    if (!products.length) {
      const fallback = await searchProducts({ keywords: 'smartphone 5G android', category: '509', limit: '10', sort: 'LAST_VOLUME_DESC' });
      return res.json({ success: true, products: fallback.success ? fallback.products : [] });
    }
    const phoneKeywords = ['phone', 'smartphone', 'mobile', 'cellphone', 'iphone', 'samsung', 'xiaomi', 'poco', 'redmi', 'realme', 'oppo', 'huawei', 'honor', 'vivo', '5g', '4g', 'android', 'ios'];
    const filtered = products.filter(p => {
      const t = (p.title || '').toLowerCase();
      return phoneKeywords.some(k => t.includes(k));
    });
    res.json({ success: true, products: filtered.length >= 3 ? filtered : products });
  } catch (e) {
    res.json({ success: false, products: [] });
  }
});
