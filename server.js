...
// ==================== Package Tracking (Cainiao + 17track) ====================
const trackingRateLimit = {};
app.get('/api/track/:trackingNumber', async (req, res) => {
  try {
    const num = req.params.trackingNumber.trim();
    if (!num || num.length < 5 || num.length > 40) {
      return res.json({ success: false, error: 'رقم تتبع غير صحيح' });
    }

    const ip = req.ip || 'unknown';
    const now = Date.now();
    if (trackingRateLimit[ip] && now - trackingRateLimit[ip].last < 3000) {
      return res.json({ success: false, error: 'انتظر قليلاً قبل المحاولة مرة أخرى' });
    }
    trackingRateLimit[ip] = { last: now };

    let result = null;

    try {
      const cainiaoResp = await axios.post('https://global.cainiao.com/global/detail.json', 
        `mailNos=${encodeURIComponent(num)}&lang=en-US&language=en-US`,
        {
          headers: {
      ...
      const cData = cainiaoResp.data;
      if (cData && cData.module && cData.module.length > 0) {
        const pkg = cData.module[0];
        if (pkg.mailNo && pkg.detailList && pkg.detailList.length > 0) {
          const events = pkg.detailList.map(ev => ({
            description: ev.desc || ev.standerdDesc || '',
            time: ev.time || ev.timeStr || '',
            location: ev.city || ev.actionCode || ''
          }));

          let status = 'transit';
          const latestDesc = (events[0]?.description || '').toLowerCase();
          if (latestDesc.includes('deliver') || latestDesc.includes('signed') || latestDesc.includes('picked up by')) {
            status = 'delivered';
          } else if (latestDesc.includes('arrive') && (latestDesc.includes('destination') || latestDesc.includes('algeria'))) {
            status = 'pickup';
          } else if (latestDesc.includes('return') || latestDesc.includes('fail') || latestDesc.includes('exception')) {
            status = 'alert';
          }

          result = {
            success: true,
            carrier: pkg.cpName || 'Cainiao / AliExpress',
            status,
            origin: pkg.originCountry || 'الصين',
      ...
      console.log('Cainiao API error:', cainiaoErr.message);
    }

    if (!result) {
      try {
        const apiResp = await axios.get(`https://api.trackingmore.com/v4/trackings/get?tracking_numbers=${encodeURIComponent(num)}`, {
          headers: {
        ...
        if (apiResp.data?.data?.length > 0) {
          const t = apiResp.data.data[0];
          const events = (t.origin_info?.trackinfo || t.destination_info?.trackinfo || []).map(ev => ({
            description: ev.StatusDescription || ev.Details || '',
            time: ev.Date || '',
            location: ev.Details || ''
          }));
          result = {
            success: true,
            carrier: t.carrier_name || 'شركة شحن',
            status: t.status === 'delivered' ? 'delivered' : t.status === 'pickup' ? 'pickup' : 'transit',
    ...
    res.json({ success: false, error: 'حدث خطأ في التتبع' });
  }
});
setInterval(() => { Object.keys(trackingRateLimit).forEach(k => { if (Date.now() - trackingRateLimit[k].last > 30000) delete trackingRateLimit[k]; }); }, 30000);
