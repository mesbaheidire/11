...
async function loadSharedCredentials() {
  ...
  // Try environment variables FIRST (Hugging Face / hosting platform)
...
function getCurrentGeminiKey() {
  ...
  // Priority: env keys (Hugging Face) > saved keys in file
  if (envKeys.length > 0) {
...
app.get('/api/spy/config', async (req, res) => {
  try {
    ...
    // متغيرات البيئة (Hugging Face) لها الأولوية على ما هو محفوظ في DB
